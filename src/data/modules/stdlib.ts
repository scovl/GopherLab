    ...
    lessons: [
      {
        ...
        vesa: {
          visaoGeral: {
            explicacao: 'Go usa interfaces para I/O: `io.Reader` e `io.Writer` são o coração — arquivos, rede, buffers, tudo implementa essas interfaces. `os.Open/Create` manipula arquivos. `bufio.Scanner` lê linha a linha. `defer file.Close()` garante cleanup. `bufio.Reader/Writer` adiciona buffer para performance, reduzindo system calls.',
      ...
      },
      {
        ...
        vesa: {
          visaoGeral: {
            explicacao: 'O pacote `net/http` fornece cliente e servidor HTTP completos. Para consumir APIs: `http.Get`, `http.Post` ou `http.NewRequest` para controle total. `encoding/json` faz Marshal (struct→JSON) e Unmarshal (JSON→struct) usando struct tags. Use `json.NewDecoder` para streams e `json.MarshalIndent` para output legível.',
